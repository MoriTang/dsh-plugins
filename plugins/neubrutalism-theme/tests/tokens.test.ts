...
describe('Neubrutalism theme', () => {
  ...
  it('keeps focus and reduced-motion behavior in the stylesheet', async () => {
    ...
    assert.match(css, /\[class\*='search'\] button\[class\*='clearButton'\]/)
    assert.match(css, /\[class\*='sectionHeader'\] button\[class\*='iconButton'\]/)
    ...
    assert.match(css, /input\[class\*='searchInput'\]:focus-visible[^{]*\{[^}]*border: 0 !important[^}]*box-shadow: none !important[^}]*outline: 0 !important/s)
